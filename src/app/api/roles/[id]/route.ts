...
export async function GET(
  ...
  try {
    ...
    const url = `${API_BASE}/api/v1/roles/${params.id}`;
    ...
    if (!res.ok) {
      ...
      return NextResponse.json(
        { error: errorData?.error || "Failed to fetch role" },
        { status: res.status }
      );
    }
...
export async function PUT(
  ...
  try {
    ...
    console.error("Roles API PUT error:", error);
    return NextResponse.json(
      { error: "Internal server error" },
      { status: 500 }
    );
...
export async function DELETE(
  ...
  try {
    ...
    if (!res.ok) {
      const errorData = await res.json().catch(() => null);
      return NextResponse.json(
        { error: errorData?.error || "Failed to delete role" },
        { status: res.status }
      );
    }
